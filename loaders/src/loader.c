...
#elif defined(STM32L1)
#define FLASH_OB_Erase() FLASH_OB_WRPConfig(OB_WRP_AllPages, DISABLE)
#endif
...
// Erase flash where needed
    ...
    for (a = StartSector; a <= EndSector; a += 8) {
      if (erased_sectors & (1 << a / 8))
        continue; // Skip sectors already erased
    ...
    for (a = 0; a <= NbrOfPage; a++) {
      if (erased_sectors >= from + (FLASH_PAGE_SIZE * a))
        continue; // Skip sectors already erased
    ...
    if (PARAMS->STATUS &
        MASK_ERR) { // If error during page delete, go back to breakpoint
      FLASH_Lock();
      continue;
    }
    ...
    while (i < PARAMS->LEN) {
      ...
      if (FLASH_PGM(PARAMS->DEST + i, mmio32(BUFFER_ADDR + i)) ==
          FLASH_COMPLETE) {
        i += FLASH_STEP;
